 ...
 * V6 : SIMPLIFICATION DEAD-BAND
 *      - Suppression du sélecteur runtime DB_RISE_FALL (0/1/2/3). Les modes
 *        0 (bypass), 1 (FED seul) et 2 (RED seul) ne protègent pas une
 *        vraie cellule de commutation : ils laissent un front sans garantie
 *        de non-recouvrement, et en plus EPWMxB retombe sur l'Action-
 *        Qualifier B, qui n'est jamais programmé dans ce fichier => sortie
 *        B figée à l'oscillo. Seul le mode RED+FED (ex-case 3) est correct
 *        et donc conservé, codé en dur, sur les 3 bras.
 *      - RED et FED restent activés une seule fois à l'init (ils ne
 *        changent plus jamais) ; seul le COMPTE (db_count, dérivé de
 *        DEADBAND_NS) est encore mis à jour en live.
 ******************************************************************************/
 ...
 *===========================================================================*/
volatile float FREQ_CARRIER = 4000.0f;   /* Hz — porteuse triangulaire     */
volatile float FREQ_SINUS   =  50.0f;    /* Hz — modulante sinusoïdale     */
volatile float AMPLITUDE    =  0.80f;    /* 0.0 … 1.0 — indice modulation  */

/*===========================================================================
 * DEAD-BAND — temps mort commun aux 3 bras (RED = FED = DEADBAND_NS)
 *   Mode RED+FED (AHC) uniquement — seul mode valable pour une cellule de
 *   commutation à deux interrupteurs complémentaires.
...
/* => 1 compte = 5 ns. Registre RED/FED sur 10 bits utiles → max 1023        */
/* comptes ≈ 5115 ns.                                                        */
...
void main(void)
{
    ...
    /* Sous-module DB d'EPWM1 — mode AHC (Active High Complementary).
     * EPWMB est généré à partir d'EPWMA (inversé), avec RED à la montée
     * et FED à la descente : seul mode retenu, RED et FED sont activés
     * une fois pour toutes et ne sont plus jamais désactivés.            */
    ...
    /* EPWM1 émet un SYNCO à chaque passage à zéro du compteur — c'est ce
     * pulse qui garde EPWM2/EPWM3 alignés sur EPWM1.                       */
    EPWM_setSyncOutPulseMode(EPWM1_BASE, EPWM_SYNC_OUT_PULSE_ON_COUNTER_ZERO);

    /* --- EPWM2 : Phase B, esclave synchronisé --- */
    ...
    /* Sous-module DB d'EPWM2 — mode AHC, bras B */
    ...
    /* --- EPWM3 : Phase C, esclave synchronisé --- */
    ...
    /* Sous-module DB d'EPWM3 — mode AHC, bras C */
    ...
    /*-----------------------------------------------------------------------
     * Boucle principale — détection de changement sur les paramètres live
     *---------------------------------------------------------------------*/
 ...
 * AVEC INJECTION DE TROISIÈME HARMONIQUE (THIPWM)
 *===========================================================================*/
__interrupt void epwm1ISR(void)
{
    isr_count++;
